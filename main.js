        ...
        for (let i = 1; i <= 5; i++) {
            htmlAcumulado += `
                <div class="custom-card-item" onclick="seleccionarDiseño('banner', '${i}')">
                    <img src="https://raw.githubusercontent.com/DonPlastico/WEB-Multiusos/main/public/img/Banners/${i}.webp" alt="Banner ${i}" loading="lazy" onerror="this.src='https://via.placeholder.com/600x300/14141c/6366f1?text=BANNER+${i}'">
                </div>
            `;
        }
        ...
        avataresLocales.forEach(avatar => {
            htmlAcumulado += `
                <div class="custom-card-item" onclick="seleccionarDiseño('avatar', '${avatar}')">
                    <img src="https://raw.githubusercontent.com/DonPlastico/WEB-Multiusos/main/public/img/Avatars/${avatar}.webp" alt="Avatar ${avatar}" loading="lazy" onerror="this.src='https://via.placeholder.com/300x300/14141c/2dd4bf?text=${avatar}'">
                </div>
            `;
        });
    ...
    itemsGaleria.forEach((item, index) => {
        ...
        if (item.type === 'video') {
            ...
            thumb.innerHTML = `
                ...
                <img src="${item.thumbnail}" alt="${item.label}" loading="lazy" onerror="this.src='https://via.placeholder.com/320x180/1a1a24/6b6b7a?text=VIDEO+NO+DISPONIBLE'">
    ...
    try {
        ...
        if (data.reparto && data.reparto.length > 0) {
            ...
            data.reparto.forEach(actor => {
                const fotoSrc = actor.foto ? actor.foto : 'https://via.placeholder.com/105x158/1a1a24/6b6b7a?text=NO+FOTO';
                castHtml += `
                            ...
                            if (!contentDiv.hasAttribute('data-loaded')) {
                                ...
                                try {
                                    ...
                                    seasonData.episodes.forEach(ep => {
                                        const epImg = ep.still_path ? `https://image.tmdb.org/t/p/w300${ep.still_path}` : 'https://via.placeholder.com/300x170/1a1a24/6b6b7a?text=NO+FOTO';
        ...
        html += `
            ...
            return `
                <div class="listado-row" data-id="${item.id}" data-tipo="${item.tipo}">
                    ...
                    <img src="${item.poster || 'https://via.placeholder.com/60x90/1a1a24/6b6b7a?text=--'}" alt="${item.titulo}" class="listado-row-poster" loading="lazy">
...
function renderizarPersona(data) {
    ...
    const foto = data.profile_path
        ? data.profile_path
        : 'https://via.placeholder.com/500x750/1a1a24/6b6b7a?text=NO+FOTO';
...
const LINKED_ACCOUNT_CONFIG = {
    discord: { icon: 'fab fa-discord', label: 'Discord', profile: id => `https://discord.com/users/${id}` },
    facebook: { icon: 'fab fa-facebook-f', label: 'Facebook', profile: id => `https://www.facebook.com/${id}` },
    github: { icon: 'fab fa-github', label: 'GitHub', profile: username => `https://github.com/${username}` },
    google: { icon: 'fab fa-google', label: 'Google', profile: () => 'https://myaccount.google.com/' },
    spotify: { icon: 'fab fa-spotify', label: 'Spotify', profile: id => `https://open.spotify.com/user/${id}` },
    twitter: { icon: 'fab fa-x-twitter', label: 'X', profile: username => `https://x.com/${username}` }
};
...
function obtenerDatosIdentidad(identity) {
    ...
    const username = identityData.global_name || identityData.preferred_username || identityData.login || identityData.display_name || identityData.name || identityData.email || 'Cuenta vinculada';
    const config = LINKED_ACCOUNT_CONFIG[provider];
    return { identityData, provider, providerUserId, username, profileUrl: config?.profile(provider === 'github' || provider === 'twitter' ? username : providerUserId) };
}

async function sincronizarCuentasOAuth(session) {
    if (!session) return;
    ...
    for (const provider of providers) {
        ...
        const button = provider === 'discord'
            ? document.getElementById('btn-link-discord')
...
async function iniciarVinculacionOAuth(provider) {
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) {
        showToast('error', 'Acceso denegado', 'Debes iniciar sesión para vincular una cuenta.');
        return;
    }
    ...
    const { data, error } = await supabase.auth.linkIdentity({
        provider,
        options: {
            redirectTo: `${window.location.origin}/editar-perfil?oauth=${provider}`,
    ...
    if (error) {
        ...
        const mensaje = manualLinkingDisabled
            ? 'Activa "Allow manual linking" en la configuración de Identity Linking de Supabase.'
            : error.message;
        showToast('error', `No se pudo vincular ${LINKED_ACCOUNT_CONFIG[provider]?.label || provider}`, mensaje);
        return;
    }
    if (data?.url) window.location.assign(data.url);
}

async function desvincularOAuth(provider) {
    const identities = await obtenerIdentidadesVinculadas();
    const identity = identities.find(item => item.provider === provider);
    if (!identity) return;
    ...
    const { data: { session } } = await supabase.auth.getSession();
    if (session) {
        await supabase.from('cuentas_vinculadas').delete()
            .eq('user_id', session.user.id)
...
function configurarVinculacionOAuth() {
        ...
        button.onclick = async () => {
            ...
            try {
                if (button.dataset.linked === 'true') {
                    await desvincularOAuth(provider);
                    showToast('success', `${LINKED_ACCOUNT_CONFIG[provider].label} desvinculado`, 'La cuenta ya no aparece en tu perfil.');
                } else {
                    await iniciarVinculacionOAuth(provider);
                }
            } catch (error) {
                console.error(`Error gestionando ${provider}:`, error);
                showToast('error', `Error con ${LINKED_ACCOUNT_CONFIG[provider].label}`, error.message);
            }
        };
    });
}
...
async function iniciarVinculacionDiscord() {
    return iniciarVinculacionOAuth('discord');
}

async function desvincularDiscord() {
    return desvincularOAuth('discord');
}
...
function crearTarjetaTrendPelicula(pelicula, posicion) {
    ...
    const posterUrl = pelicula.poster || 'https://via.placeholder.com/180x270/14141c/6366f1?text=SIN+POSTER';
...
function crearTarjetaTrendSerie(serie, posicion) {
    ...
    const posterUrl = serie.poster || 'https://via.placeholder.com/180x270/14141c/6366f1?text=SIN+POSTER';
...
function crearTarjetaTrendTrailer(item, tipo, posicion) {
    ...
    if (tipo === 'game') {
        ...
        posterUrl = item.cover && item.cover.url
            ...
            : 'https://via.placeholder.com/180x270/14141c/6366f1?text=SIN+POSTER';
        ...
        posterUrl = item.poster || 'https://via.placeholder.com/180x270/14141c/6366f1?text=SIN+POSTER';
        fecha = item.fecha
...
// ==========================================================================
//   CARGA DINÁMICA DE ITEMS DE LISTA (VERSIÓN OPTIMIZADA CON RPC)
// ==========================================================================
...
async function cargarItemsLista(listaId, resetear = true) {
    ...
    try {
        ...
        const { data: items, error: rpcError } = await supabase.rpc('get_items_lista_filtrados', {
            ...
            p_media_tipo: listaTipoActual,   // 'movie', 'tv', 'game' o 'mixta'
        ...
        if (mensajeFinal) {
            ...
            mensajeFinal.textContent = `${listaItemsTotal} ${tipoLabel} (filtro: ${estadoSeleccionado})`;
        }
...
async function enriquecerItemsListaCompleto(items) {
    ...
    // Caché en sessionStorage para no repetir peticiones
    const cacheKey = 'lista_items_cache';
    let cache = JSON.parse(sessionStorage.getItem(cacheKey) || '{}');

    // Aumentamos la concurrencia a 20 para ir mucho más rápido
    const batchSize = 20;
    ...
    for (let i = 0; i < items.length; i += batchSize) {
        ...
        await Promise.all(batch.map(async (item) => {
            ...
            if (cache[key]) {
                listaItemsEnriquecidos[key] = cache[key];
                return;
            }

            if (listaItemsEnriquecidos[key]) return;
            ...
            try {
                ...
                console.error(`❌ Error enriqueciendo ${key}:`, e);
                listaItemsEnriquecidos[key] = {
                    id: item._media_id,
                    tipo: item._media_tipo,
                    titulo: `ID: ${item._media_id}`,
...
// En la sección de listeners de filtros de lista
document.querySelectorAll('input[name="estado-lista-filtro"]').forEach(radio => {
    radio.addEventListener('change', () => {
        // Recargar la lista con el nuevo filtro (filtrado en servidor vía RPC)
        if (listaIdActual) {
...
window.aplicarFiltrosListaDetalle = function () {
    ...
    const tarjetas = Array.from(grid.children)
        .filter(t => t.id !== 'lista-detalle-loader' && t.id !== 'lista-detalle-end');

    tarjetas.forEach(tarjeta => {
        const tituloEl = tarjeta.querySelector('.game-title, .list-card-title, .film-titulo');
        if (tituloEl && textoBuscador) {
            tarjeta.style.display = tituloEl.textContent.toLowerCase().includes(textoBuscador) ? '' : 'none';
        } else {
