exports.handler = async (event, context) => {
    ...
    const tipo = event.queryStringParameters.tipo || 'movie';
    const id = event.queryStringParameters.id;
    const busqueda = event.queryStringParameters.query || '';
    const page = parseInt(event.queryStringParameters.page) || 1;
    const includeAdult = event.queryStringParameters.adult === 'true' ? 'true' : 'false';
    const minVotes = parseInt(event.queryStringParameters.minVotes) || 0;
    ...
    const genero = event.queryStringParameters.genero || '';
    const generos = event.queryStringParameters.generos || '';
    ...
    try {
        ...
        // =========================================================
        if (tipo === 'tv_season' && id && event.queryStringParameters.season) {
            const seasonNum = event.queryStringParameters.season;
